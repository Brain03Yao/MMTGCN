...
class Weighted_Margin(Module):
    ...
    def forward(self, outputa1, outputa2, outputb1, outputb2,
                outputc1, outputc2, outputd1, outputd2, m):

        target = torch.cuda.FloatTensor(outputa1.size()).fill_(-1)
        loss = self.w1 * m(outputa1, outputa2, target) + self.w2 * m(outputb1, outputb2, target) \
            + self.w3 * m(outputc1, outputc2, target) + self.w4 * m(outputd1, outputd2, target)
...
def accuracy_new(dista1, dista2, distb1, distb2, distc1, distc2, distd1, distd2, margin=5.0):
    # distb represent the distance between anchor and negative sample
    pred = (0.25 * (dista2 - dista1 - margin) + 0.25 * (distb2 - distb1 - margin) +
            0.3 * (distc2 - distc1 - margin) + 0.2 * (distd2 - distd1 - margin)).cpu().data
    pred = pred.view(pred.numel())
    pred = (pred > 0).float()
    return pred.sum()*1.0/pred.numel()


def accuracy_new_add(dista1, dista2, distb1, distb2, distc1, distc2, distd1, distd2, label_ndarray, margin=5.0):
    # distb represent the distance between anchor and negative sample
    pred = (0.25*(dista2 - dista1 - margin) + 0.25*(distb2 - distb1 - margin) +
            0.3*(distc2 - distc1 - margin) + 0.2*(distd2 - distd1 - margin)).cpu().data
    pred = pred.view(pred.numel())
